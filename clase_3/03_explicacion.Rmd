...
#### Boxplot de rendimientos por campaña

Hacemos un procesamiento simple: Seleccionamos el cultivo de interés (Soja) y las útlimas campañas (id.campaña).    
Es importante que las variables sean del tipo que conceptualmente les corresponde (La Campaña es una variable categórica, no continua), para que el ggplot pueda graficarlo correctamente. 
...
class(estimaciones$Campana)
class(estimaciones$Rendimiento..Kg.Ha.)

ggdata <-  estimaciones %>% 
  filter(Cultivo=="Soja total", ID.Campaña>30 & Provincia %in% c("BUENOS AIRES", "SANTA FE", "CORDOBA")) %>%   mutate(Rendimiento..Kg.Ha. = as.numeric(Rendimiento..Kg.Ha.))
...
ggplot(ggdata, aes(x = Campana, y = Rendimiento..Kg.Ha.)) +
  geom_boxplot()+
  scale_y_continuous(limits = c(0, 5500))+
  theme(axis.text.x = element_text(angle = 90))
```

Si queremos agregar la dimensión _Provincia_, podemos hacer un ```facet_wrap()```

```{r}

ggplot(ggdata, aes(y = Rendimiento..Kg.Ha.)) +
  geom_boxplot()+
  scale_y_continuous(limits = c(0, 6000))+
  facet_wrap(~ Provincia, labeller = "label_both")+
  theme(axis.text.x = element_text(angle = 90))
```

Por la forma en que está presentado el gráfico, el foco de atención sigue puesto en las diferencias de rendimiento histricas entre provincias. 
...
ggdata %>% filter(Campana %in% c("2017/18","2016/17")) %>% 
  ggplot(., aes( y = Rendimiento..Kg.Ha., fill = Campana )) +
  geom_boxplot()+
  scale_y_continuous(limits = c(0, 5000))+
  facet_grid(~ Provincia, labeller = "label_both") 
