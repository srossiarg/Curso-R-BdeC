...
Tambien se creo recientemente el repositorio [IDERA](https://www.idera.gob.ar/)
...
* `geometry type:  MULTIPOLYGON`: los archivos con información geográfica contienen colecciones de puntos, de líneas, o de polígonos. En éste caso son polígonos; tiene sentido para la información que esperamos, que son los departamentos de Argnetina.
...
La función `plot` aplicada sobre la saldida de la función `st_geommetry` nos grafica la geomtria del dataset.

```{r fig.height = 8, fig.width = 5}
plot(st_geometry(departamentos), col = NA, border = "grey",axes = TRUE)
```
La fiigura parece un poco apretada. Veamos el CRS de esta figura
```{r}
st_crs(departamentos)
```
Es la proyeccion Mercator. Que distorsiona un poco las ditancias lejanas al Ecuador.


Si quisieramos descartar la Antartida Argentina para concentrarnos en el continente tenemos que recortar el bounding box.
...
Pero deciamos que la proyeccion mercator distorsiona un poco el tamaño del país. Si quisieramos cambiar la proyeccion por la oficial que es la gauss-kruger o una aproximación ([Transverse Mercator](https://epsg.io/5345)). Debemos aplicar `st_transform` con el codgo crs correspondiente a la transfomración que deseamos.
...
Mapas choropleth se aplcia la función `choroLayer`. Al igual que con ggplot los gráficos se construyen por capas. Tampoco es necesario recordar el código de memoria. Cuando haya que usarlo podemos visitar el [machete](http://riatelab.github.io/cartography/vignettes/cheatsheet/cartography_cheatsheet.pdf).
...
Combinandolo con Tydiverse podemos transformar los datos facilmente y mapearlos. Por ejemplo, a continuación cambiamos la unidades y el nombre de la variable que registra el rendimiento de la soja.
```{r fig.height = 8, fig.width = 6}
...
Tambien se pueden hacer mapas con más chiches
