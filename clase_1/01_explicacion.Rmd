...
CAMPANA  <-  c("2015/16","2015/16","2015/16",
...
```{r error=TRUE}
Datos[Datos$CAMPANA=="2015/16",]
```

La lógica del paso anterior sería: Accedo al dataframe ```Datos```, pidiendo únicamente conservar las filas (por eso la condición se ubica a la _izquierda_ de la  ```, ```) que cumplan el requisito de pertenecer a la categoría __"2015/16"__ de la variable __CAMPANA__.    

Aún más, podría aplicar el filtro y al mismo tiempo identificar una variable de interés para luego realizar un cálculo sobre aquella. Por ejemplo, podría calcular la media de los rindes en CAMPANA 2015/16. 

```{r error=TRUE}
###Por separado
rindes_15_16 <- Datos$RINDE[Datos$CAMPANA=="2015/16"]
rindes_15_16
```
...
```{r error=TRUE}
### Todo junto
mean(Datos$RINDE[Datos$CAMPANA=="2015/16"])
```

La lógica de esta sintaxis sería: "Me quedo con la variable __RINDE__, cuando la variable CAMPANA sea igual a __"2015/16"__, luego calculo la media de dichos valores".
